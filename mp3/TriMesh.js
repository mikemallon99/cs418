 ...
 */
    ...
    */
    computeAABB(){
      ...
      // Loop through every vertex and find the max/min coords
      var v = [0,0,0];
    ...
    loadFromOBJ(fileText)
    {
        ...
        for (i=0; i < lines.length; i++) {
          if(lines[i][0] == '#') {
            ...
            // Collect vertices
            let parsedVerts = lines[i].split(' ');
            this.vBuffer.push(parseFloat(parsedVerts[1]));
            this.vBuffer.push(parseFloat(parsedVerts[2]));
            this.vBuffer.push(parseFloat(parsedVerts[3]));
            verts += 1;
          }
          else if (lines[i][0] == 'f') {
            // Collect faces (accounting for the double space)
            let parsedFaces = lines[i].split(' ');
            this.fBuffer.push(parseInt(parsedFaces[2])-1);
            this.fBuffer.push(parseInt(parsedFaces[3])-1);
            this.fBuffer.push(parseInt(parsedFaces[4])-1);
            faces += 1;
          }
        }
